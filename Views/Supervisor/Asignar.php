			...
			<?php I18n::boot(); $flashes = Flash::peek(); ?>
						<?php foreach ($flashes as $f): ?>
							...
							<div class="alert alert-<?php echo htmlspecialchars($f['type']); ?>">
								<?php echo htmlspecialchars(__($f['message'])); ?>
							</div>
						<?php endforeach; ?>
		...
		<div class="asignar-panel">
			<form action="/ProyectoPandora/Public/index.php?route=Supervisor/Asignar" method="get" id="filtros" class="filters asignar-filters">
				...
				<div class="field asignar-field">
					<label class="asignar-label"><?= __('supervisor.assign.filter.availability'); ?></label>
					<select name="estado" id="estado" class="asignar-input">
						<?php $estadoSel = $_GET['estado'] ?? 'todos'; ?>
						<option value="todos" <?php echo $estadoSel==='todos'?'selected':''; ?>><?= __('supervisor.assign.filter.option.all'); ?></option>
						<option value="Disponible" <?php echo $estadoSel==='Disponible'?'selected':''; ?>><?= __('supervisor.assign.filter.option.available'); ?></option>
						<option value="Ocupado" <?php echo $estadoSel==='Ocupado'?'selected':''; ?>><?= __('supervisor.assign.filter.option.busy'); ?></option>
					</select>
				</div>
				<div class="field asignar-field asignar-field--grow">
					<label class="asignar-label"><?= __('supervisor.assign.search.tech'); ?></label>
					<input type="text" name="q" id="q" placeholder="<?= __('supervisor.assign.search.placeholder'); ?>" value="<?php echo htmlspecialchars($_GET['q'] ?? ''); ?>" class="asignar-input" />
				</div>
				<div class="field asignar-field">
					<label class="asignar-label"><?= __('supervisor.assign.ticket.label'); ?></label>
					<select name="ticket_id" id="ticket_id" class="asignar-input">
						<?php if (empty($ticketsSinTecnico)): ?>
							<option value=""><?= __('supervisor.assign.ticket.none'); ?></option>
						<?php else: ?>
							<option value=""><?= __('supervisor.assign.ticket.select'); ?></option>
				...
				<div class="actions asignar-actions">
					<button type="submit" class="btn btn-primary"><?= __('supervisor.assign.actions.apply'); ?></button>
					<a href="/ProyectoPandora/Public/index.php?route=Supervisor/Asignar" class="btn btn-outline"><?= __('supervisor.assign.actions.clear'); ?></a>
				</div>
			</form>
			...
			<div class="asignar-grid">
				<?php if (empty($filtrados)): ?>
					<div class="asignar-empty"><?= __('supervisor.assign.empty'); ?></div>
				<?php else: ?>
							...
							<div class="asignar-card__head">
								...
								<div class="asignar-card__title">
									<div class="asignar-card__row">
										...
										<span class="badge <?php echo $estado==='Disponible' ? 'badge--success' : ($estado==='Ocupado' ? 'badge--danger' : 'badge--muted'); ?>" title="<?= __('supervisor.assign.badge.state.hint'); ?>"><?php echo htmlspecialchars(__('supervisor.assign.status.' . (strcasecmp($estado,'Disponible')===0?'available':(strcasecmp($estado,'Ocupado')===0?'busy':'unknown')))); ?></span>
									</div>
									...
									<div class="asignar-card__subtitle"><?= __('supervisor.assign.field.specialty'); ?>: <?php echo htmlspecialchars($tec['especialidad'] ?? '—'); ?></div>
								</div>
							</div>
							<div class="asignar-card__chips">
								<div class="chip"><?= __('supervisor.assign.chip.assigned'); ?>: <?php echo (int)($tec['tickets_asignados'] ?? 0); ?></div>
								<div class="chip"><?= __('supervisor.assign.chip.active'); ?>: <?php echo (int)($tec['tickets_activos'] ?? 0); ?></div>
								<div class="chip"><?= __('supervisor.assign.chip.email'); ?>: <?php echo htmlspecialchars($tec['email'] ?? ''); ?></div>
							</div>
							...
							<form action="/ProyectoPandora/Public/index.php?route=Supervisor/AsignarTecnico" method="post" class="asignar-assign-form">
								...
								<select name="ticket_id" class="asignar-input asignar-input--small" <?php echo (!$dispOk || empty($ticketsSinTecnico)) ? 'disabled' : ''; ?>>
									<?php if (empty($ticketsSinTecnico)): ?>
										<option value=""><?= __('supervisor.assign.ticket.none'); ?></option>
									<?php else: ?>
										<option value=""><?= __('supervisor.assign.ticket.selectShort'); ?></option>
										<?php foreach ($ticketsSinTecnico as $t): ?>
								...
								<button type="submit" class="btn btn-primary" <?php echo (!$dispOk || empty($ticketsSinTecnico)) ? 'disabled' : ''; ?>><?= __('supervisor.assign.actions.assign'); ?></button>
							</form>
						</div>
					<?php endforeach; ?>
				<?php endif; ?>
